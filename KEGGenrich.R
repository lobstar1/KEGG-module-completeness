...
keggAnn <- read.table("/nfs_genome/BioDB/kegg/202108/kegg/pathway/map_title.tab",header=F,sep="\t")
...
pv.out <- pathview_u(gene.data = fcKO, pathway.id = path ,species = "ko", out.suffix = opt$output, same.layer = T, low = list(gene = "green", cpd = "blue"), mid = list(gene = "gray", cpd = "gray"), high = list(gene = "red", cpd = "yellow"), na.col = "transparent", kegg.native = T, kegg.dir="/nfs_genome/BioDB/kegg/202108/kegg/xml/kgml/ko/")
pv.out <- pathview(gene.data = fcKO, pathway.id = path ,species = "ko", out.suffix = opt$output, same.layer = T, kegg.native = F, kegg.dir="/nfs_genome/BioDB/kegg/202108/kegg/xml/kgml/ko/")
